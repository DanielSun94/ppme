...
from preprocess_config import renmin_mapping_file, standard_mapping_file, joint_save_path, label_mapping_save_path
import os
...
def diagnosis_tree_generate_standard(mapping_file):
    ...
    with open(mapping_file, 'r', encoding='utf-8-sig') as f:
        ...
        for line in islice(csv_reader, 1, None):
            assert len(line) == 11
            code_1, name_1, code_2, name_2, code_3, name_3, code_4, name_4, code_5, name_5 = line[1:11]
            ...
            code_5 = code_5.replace('\\n', '').replace('\n', '').replace('\t', '').replace('\0', '').replace('−', '')
            name_5 = name_5.replace('\\n', '').replace('\n', '').replace('\t', '').replace('\0', '').replace('−', '')

            for (key_1, key_2, key_3, key_4, key_5), mapping_dict in \
                    zip(((code_1, code_2, code_3, code_4, code_5), (name_1, name_2, name_3, name_4, name_5)),
                        (code_mapping_dict, name_mapping_dict)):
                if key_1 not in mapping_dict:
                    mapping_dict[key_1] = dict()
                ...
                if key_2 != "" and key_3 != "" and key_3 not in mapping_dict[key_1][key_2]:
                    mapping_dict[key_1][key_2][key_3] = dict()
                if key_2 != "" and key_3 != "" and key_4 != '':
                    mapping_dict[key_1][key_2][key_3][key_4] = set()
                if key_2 != "" and key_3 != "" and key_4 != '' and key_5 != "":
                    mapping_dict[key_1][key_2][key_3][key_4].add(key_5)

            for index, code, name in ((0, code_1, name_1), (1, code_2, name_2), (2, code_3, name_3),
                                      (3, code_4, (name_3, name_4)), (4, code_5, (name_3, name_4, name_5))):
                if isinstance(name, str) and name == '':
                    continue
                if isinstance(name, tuple) and len(name) == 2 and name[1] == '':
                    continue
                if isinstance(name, tuple) and len(name) == 3 and name[2] == '':
                    continue
...
def construct_label(diagnosis, item_list):
    ...
    text = ("请假设你是一名临床医生，你需要将诊断进行分类。\n"
            ...
            '注意。\n'
            '1.你需要把诊断映射到最为相似的类型，并且只能输出一个结果。如果你觉得没有特别合适的选择，而选项中包含“其它类型的疾病”这样的表述，'
            '请选择这个。如果你觉得没有任何一个合适，请随机选择一个。'
            '2.如果清单中只有一个选择，则不管这个选择看上去合不合理，都选它。'
            '3.如果你的诊断中实际包含了多种疾病，例如“高血压;糖尿病”，请只考虑第一个疾病（高血压）的分类，忽略后续的。'
            )
    return text
...
def label_generation(diagnosis_list, llm_model, llm_tokenizer, batch_size, model_config_dict, name_mapping_dict,
    ...
    previous_success_pool = load_cache(save_path)
    ...
    while not label_generation_manager.has_complete():
        ...
        if step % save_per_step == 0:
            save_cache(label_generation_manager, save_path)
    logger.info('success')
    save_cache(label_generation_manager, save_path)
...
class LabelGenerationManager(object):
    ...
    def update_manager(self, result_list, diagnoses_list, show_result):
        ...
        for item in self.data_pool:
            ...
            for i, diagnosis in enumerate(item[1]):
                # 此处叶子节点判断有两种case，第一种是depth到最底，也就是理论上的最深节点，也就是这里的else
    ...
    def convert_to_code(self, diagnosis_category):
        ...
        elif len(diagnosis_category) == 4:
            key = diagnosis_category[-2] + '->' + diagnosis_category[-1]
        else:
            assert len(diagnosis_category) == 5
            key = diagnosis_category[-3] + '->' + diagnosis_category[-2] + '->' + diagnosis_category[-1]
    ...
    def parse_result(result_list, diagnoses_list):
        ...
        for i, (result, disease) in enumerate(zip(result_list, diagnoses_list)):
            ...
            if pattern_list is None or len(pattern_list) == 0 or len(pattern_list) > 1:
                ...
                index = int(pattern_list[0].strip()) - 1
...
def read_data(file_path):
    diagnosis_dict = dict()
    line_count = 0
    def add_diagnosis(diagnosis_str):
        diagnosis_list = diagnosis_str.split('$$$$$')
        for item in diagnosis_list:
            if len(item) > 0:
                if item not in diagnosis_dict:
                    diagnosis_dict[item] = 0
                diagnosis_dict[item] += 1

    with open(file_path, 'r', encoding='utf-8-sig') as f:
        csv_reader = csv.reader(f)
        for line in islice(csv_reader, 1, None):
            line_count += 1
            first_page, discharge_diagnosis, table_discharge = line[8:11]
            if first_page != 'None':
                add_diagnosis(first_page)
            if discharge_diagnosis != 'None':
                add_diagnosis(discharge_diagnosis)
            if table_discharge != 'None':
                add_diagnosis(table_discharge)

    diagnosis_list = sorted([[key, diagnosis_dict[key]] for key in diagnosis_dict], key=lambda x: x[1], reverse=True)
    return diagnosis_list
...
def main():
    ...
    diagnosis_list = read_data(joint_save_path)
    logger.info('diagnosis list loaded, full data size: {}'.format(len(diagnosis_list)))
    with open(os.path.abspath('../resource/diagnosis_name_count.csv'), 'w', encoding='utf-8-sig') as f:
        csv.writer(f).writerows(diagnosis_list)
    ...
    diagnosis_name_list = [item[0] for item in diagnosis_list]
    label_generation(
        diagnosis_name_list, llm_model, llm_tokenizer, batch_size, model_config_dict, name_mapping_dict,
