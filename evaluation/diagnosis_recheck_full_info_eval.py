...
import json
from util import (candidate_srrsh_analysis, candidate_mimic_analysis,
                  read_chinese_icd, read_english_icd, eval_performance)
from evaluation_config import (evaluation_final_diagnosis_eval, full_info_eval_folder)
...
def read_data(folder, source_llm, key):
    target_folder = os.path.join(folder, source_llm, key)
    data_dict = dict()
...
def main():
    ...
    # openbiollm llama_3_3_70b ultra_medical_llm local_qwen_2__5_72b_int4_2 deepseek_r1_70b huatuogpt_o1_72b
    source_llm_name = 'huatuogpt_o1_72b'
    data_source = 'mimic'
    doctor_type = 'llm'
    start_index = 0
    end_index = 1250
    logger.info(f'language: {language}')
    key_list = [data_source, doctor_type, language, rank_llm_name, source_llm_name]

    key = data_source
    target_folder = os.path.join(evaluation_final_diagnosis_eval, rank_llm_name,
                                 'full_info_source_' + source_llm_name, key)
    ...
    # for normal eval
    key = data_source
    data_dict = read_data(full_info_eval_folder, source_llm_name, key)
    # for cross eval
    ...
    for key in ordered_list:
        ...
        parse_index += 1
        candidate, data = data_dict[key]['diagnosis'], data_dict[key]['data']
        ...
        if parse_index % 20 == 0:
            eval_performance(target_folder, 2500)
    eval_performance(target_folder, 2500)
