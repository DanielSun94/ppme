...
import traceback
import re
from .llm_doctor_prompt import llm_prompt_dict
...
def generate_high_risk_diseases(dialogue_string, call_llm, client_id, language):
    ...
    while not success_flag:
        try:
            ...
            result_list = parse_result_disease(response)
            success_flag = True
        except:
            failed_time += 1
            trace = traceback.format_exc()
            ...
            if failed_time > call_llm_retry_time:
                ...
                break
    logger.info(f'client: {client_id}, result_list: {result_list}')
    return response, result_list, question_type
...
def screen_information_sufficiency_analyze(dialogue_string, call_llm, client_id, language):
    ...
    logger.info(f'client: {client_id}, screen sufficiency test, result: {sufficient_flag}, response: {response}')
...
def diagnosis_procedure(dialogue_string, call_llm, client_id, disease_name, question_num,
    ...
    response = get_response(prompt, call_llm, client_id, question_type)
    if '你确诊了{}'.format(disease_name) in response:
        complete_flag = 1
        confirm_flag = 1
    elif '你没有得{}'.format(disease_name) in response:
        complete_flag = 1
        confirm_flag = 0
    else:
        complete_flag = 0
    ...
    if question_num + 1 >= maximum_question_per_differential_diagnosis_disease:
        complete_flag = 1
    return response, complete_flag, confirm_flag, question_num+1
