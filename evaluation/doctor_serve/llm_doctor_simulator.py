import json
from .llm_doctor_util import (dialogue_organize, screen_information_sufficiency_analyze, screen_question_generation,
...
def doctor_agent_behavior(messages,
    ...
    注意，llm doctor的两段式提问会强行分成两段处理，初筛阶段会一定问完预设的n个问题
    llm作为baseline不处理答非所问这种情况，额外询问医学或者非医学问题的情况
    llm agent behavior的一大特点是，不是和ecdai那样是生成prompt，他是直接生成response，决策链相对简单
    """
    ...
    if previous_end_flag == 1 or turn_num > 15:
        ...
        if screen_flag == 1:
            ...
            if len(dialogue_string) > 0:
                sufficient_flag = screen_information_sufficiency_analyze(dialogue_string, call_llm, client_id,
                                                                         environment_language)
            else:
                sufficient_flag = 0
            ...
            if sufficient_flag == 0:
                response, question_type = screen_question_generation(dialogue_string, call_llm, client_id,
                                                                     environment_language)
            else:
                assert sufficient_flag == 1
                response, candidate_disease_list, question_type = (
        ...
        if phase != 'SCREEN' and screen_flag == 0:
            ...
            for i in range(top_diagnosis_disease_num):
                ...
                if complete_flag == 0:
                    response, complete_flag, confirm_flag, question_num = (
